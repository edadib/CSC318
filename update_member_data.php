...
$email = $_POST['email'];
$update = mysqli_query($connection, "UPDATE staff SET name='$name', gender='$gender', dob='$dob', address='$address', telephone='$telephone', email='$email' WHERE icno='$icno'") or die(mysqli_error()); // query for update data in database
/*if($update){ // if update query execution successfull
header("Location: update_member_data.php?icno=".$icno."&process=success"); // add process-success in URL
...
echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Cannot update data, please try again. <a href="member_profile.php"><- Back</a></div>'; // display cannot update message'
}*/
}
?>
<!-- Form for collecting member data -->
<form class="form-horizontal" action="update_member_data.php" method="post" enctype="multipart/form-data">
